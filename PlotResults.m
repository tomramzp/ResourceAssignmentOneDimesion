...
clear Nuser_beam_c_aux  Req_user_c_aux R_off_POW_c_aux ...
    R_off_BW_c_aux R_off_MAP_c_aux R_off_BW_MAP_c_aux R_off_BW_POW_c_aux ...
    user_beams_c_aux 
...
tech_label={'POW', 'BW', 'MAP','BW-MAP','BW-POW'};
...
sim_sol= find([en_POW en_BW en_MAP en_BW_MAP en_BW_POW]);
...
colors(4,:)=[0.4660 0.6740 0.1880] ;
colors(5,:)=[0.4940 0.1840 0.5560];
...
if en_MAP
    NQU_sims{3}=cellfun(@(x,y) sum(NQU_fun(x,y)),Req_user_c,R_off_MAP_c);
    NQU_av(3)=mean(NQU_sims{3});
    
end

if en_BW_MAP
    NQU_sims{4}=cellfun(@(x,y) sum(NQU_fun(x,y)),Req_user_c,R_off_BW_MAP_c);
    NQU_av(4)=mean(NQU_sims{4});
end

if en_BW_POW
    NQU_sims{5}=cellfun(@(x,y) sum(NQU_fun(x,y)),Req_user_c,R_off_BW_POW_c);
...
if en_MAP
    NU_sims{3}=cellfun(@(x,y) sum(NU_fun(x,y)),Req_user_c,R_off_MAP_c);
    NU_av(3)=mean(NU_sims{3});
    
end

if en_BW_MAP
    NU_sims{4}=cellfun(@(x,y) sum(NU_fun(x,y)),Req_user_c,R_off_BW_MAP_c);
    NU_av(4)=mean(NU_sims{4});
end

if en_BW_POW
    NU_sims{5}=cellfun(@(x,y) sum(NU_fun(x,y)),Req_user_c,R_off_BW_POW_c);
...
if en_MAP
    Offered_sims{3}=cellfun(@(x) Offered_fun(x),R_off_MAP_c);
    Offered_av(3)=mean(Offered_sims{3});
    
end

if en_BW_MAP
    Offered_sims{4}=cellfun(@(x) Offered_fun(x),R_off_BW_MAP_c);
    Offered_av(4)=mean(Offered_sims{4});
end

if en_BW_POW
    Offered_sims{5}=cellfun(@(x) Offered_fun(x),R_off_BW_POW_c);
...
if en_MAP
    Min_user_sims{3}=cellfun(@(x)Min_user_fun(x),R_off_MAP_c);
    Min_user_av(3)=mean(Min_user_sims{3});
    
end

if en_BW_MAP
    Min_user_sims{4}=cellfun(@(x)Min_user_fun(x),R_off_BW_MAP_c);
    Min_user_av(4)=mean(Min_user_sims{4});
end

if en_BW_POW
    Min_user_sims{5}=cellfun(@(x)Min_user_fun(x),R_off_BW_POW_c);
...
if en_MAP
    aux_Rates{3}=total_band/1e6*cell2mat(R_off_MAP_c(:));
end

if en_BW_MAP
    aux_Rates{4}=total_band/1e6*cell2mat(R_off_BW_MAP_c(:));
end

if en_BW_POW
    aux_Rates{5}=total_band/1e6*cell2mat(R_off_BW_POW_c(:));
end
...
figure,hold on
plot(total_band/1e6*[R_av R_av],[0 1],'k','LineWidth',2,'DisplayName','Requested User Traffic')
for ind_tech=sim_sol
...
for ind_sim=1:Nsims
    ...
    for i=1:K
        ...
        if en_MAP
            Offer_beam{3}(ind_sim,i)= total_band/1e6*sum ( R_off_MAP_c{ind_sim}(user_index));
        end
        
        if en_BW_MAP
            Offer_beam{4}(ind_sim,i)= total_band/1e6*sum ( R_off_BW_MAP_c{ind_sim}(user_index));
        end
        
        if en_BW_POW
             Offer_beam{5}(ind_sim,i)= total_band/1e6*sum ( R_off_BW_POW_c{ind_sim}(user_index));
        end
...
tech_label2={'Offered: POW', 'Offered: BW', 'Offered: MAP','Offered: BW-MAP','Offered: BW-POW'};
...
T = table(NQU_av',NU_av',Offered_av',Min_user_av','VariableNames',{'NQU','NU','Offered Rate [Mps]','Min. User Rate [Mbps]'},'RowName',{'POW','BW','MAP','BW-MAP','BW-POW'});
disp(T)
